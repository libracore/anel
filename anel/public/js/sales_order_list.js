// Coypright (c) 2025, libracore and contributors
// For licence information, please see license.txt
...
frappe.listview_settings['Sales Order'] = {
    ...
    'onload': function(listview) {
        ...
        listview.page.add_menu_item(__("Rechnungen erstellen"), function() {
            let selected = listview.get_checked_items();
            create_invoices(selected);
        });
    }
};

function create_invoices(selected) {
    // compile order list
    let selected_orders = [];
    for (let i = 0; i < selected.length; i++) {
        selected_orders.push(selected[i].name);
    }
    
    frappe.call({
        'method': 'anel.anel.utils.async_create_invoices',
        'args': {
            'sales_orders': selected_orders
        },
        'callback': function(response) {
            frappe.show_alert("Erstellung im Hintergrund läuft...");
        }
    });
