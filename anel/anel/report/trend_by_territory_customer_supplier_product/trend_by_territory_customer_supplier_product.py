...
def get_data(filters):
    ...
    sql_query = """SELECT 
        ...
        IFNULL(`data_P2`.`qty`, 0) AS `qty_p2`,
        IFNULL((`data_P1`.`qty` / ABS((DATEDIFF("{p1_from}", "{p1_to}")) / 30)), 0) AS `qty_p1_mt`,
        IFNULL((`data_P2`.`qty` / ABS((DATEDIFF("{p2_from}", "{p2_to}")) / 30)), 0) AS `qty_p2_mt`,
        IF(`data_P2`.`qty` != 0,
           (100 * `data_P1`.`qty`) / (`data_P2`.`qty`),
           0) AS `change`,
    ...
    (
        SELECT 
            ...
            CONCAT(`tabSales Invoice`.`territory`, "::", 
                ...
                `tabSales Invoice Item`.`item_code`) AS `key`
    ...
    LEFT JOIN 
        (SELECT 
            ...
            `tiP1`.`item_code` AS `item_code`,
            CONCAT(`tP1`.`territory`, "::", 
                `tP1`.`customer`, "::",
                `tSI1`.`supplier`, "::",
                `tiP1`.`item_code`) AS `key`,
    ...
    LEFT JOIN 
        (SELECT 
            ...
            `tiP2`.`item_code` AS `item_code`,
            CONCAT(`tP2`.`territory`, "::", 
                `tP2`.`customer`, "::",
                `tSI2`.`supplier`, "::",
                `tiP2`.`item_code`) AS `key`,
